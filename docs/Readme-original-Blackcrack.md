<h1>Plasma SVG Win7 Theme</h1>
<h2>Let There be ... Classic !</h2> => Windows 7 SVG 
 [Blackysgate.de](https://www.blackysgate.de) 
 Scalable 7 Icons for Plasma 5/6^ from Blackysgate.de - Letz there be Classic ! - with extra PNG's 


 the Win7 never Exist but the Ghost of it .. exist still ,
...
Weather Applet Icons by  https://www.deviantart.com/jackseller/art/Weather-Icons-611424540
as png's (pngintosvg) and have adjust and add couples Weathericons to make it more fit for KDE Plasma Applets.

<h2> Technical : </h2>
  

 To all WM's Developers, hold your SVG Display Library up to date !There be Icons with hidden symbols in the svg-Pictures,
 as Example, the '+' and '-' in one like in add.svg and there is the '-' as hidden market/declard and only the '+'
 is viewable or should be viewable and the '-' is not viewable, this works only with a more new svgdisplay library
 (looking to the tde-developers !)
 Dear TDE-Developers,
 you are should look to update ever fine your svg-library to have actual svg-command sentences in your svg-display library
 because these icons be made with Inkscape 1.3.2 with more new command-sequenses or so, these should supported like in Plasma 6^
 best :)
 ...
 batchscript in this themefolder for adding extra Mimetype-regestration filetypes where
 traditional not include in mimetypes of Linux, once because 'windowshate' 
 ...
 then open a konsole or xterm windows and execute it, behind you have make 'chmod 750 addmimetouser.sh'
 then have you extra icons and the theme supports the Winnt, sys, dll, cpl icons in/by this theme
 (or my NT themes where i have collected)
...
<p> </p>
  ...
  * drive-network-nfs + drive-network-nfs-umount
  * drive-network-smb + drive-network-smb-umount
  * Fallback drive-network.svg and drive-network-umount.svg if fs drive not exist for hold te Compatibility for more older Themes.
